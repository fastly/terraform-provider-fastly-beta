...
func (l *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "List all Log Shuttle logging endpoints across all Fastly CDN and Compute services at their active version, or latest version when no active version exists.",
...
func (l *ListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	tflog.Debug(ctx, "Listing Fastly Log Shuttle logging endpoints")
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, svc := range services {
			...
			loggers, err := l.client.ListLogshuttles(ctx, &fastly.ListLogshuttlesInput{
				ServiceID:      serviceID,
				ServiceVersion: version,
			})
			if err != nil {
				tflog.Warn(ctx, "Error listing Log Shuttle logging endpoints for service", map[string]any{
					"service_id": serviceID,
					"error":      err.Error(),
				})
				continue
			...
			for _, ls := range loggers {
				...
				result.DisplayName = service.ToGeneratedResourceName(fastly.ToValue(svc.Name), serviceID, *ls.Name)
				result.Diagnostics.Append(result.Identity.Set(ctx, &IdentityModel{
					ServiceID: types.StringValue(serviceID),
					Name:      types.StringValue(fastly.ToValue(ls.Name)),
				})...)

				if req.IncludeResource {
					result.Diagnostics.Append(setResourceAttrs(ctx, &result, ls, serviceID, version, fastly.ToValue(svc.Type))...)
				}
...
// accept.
func setResourceAttrs(ctx context.Context, result *list.ListResult, ls *fastly.Logshuttle, serviceID string, version int, serviceType string) diag.Diagnostics {
	var diags diag.Diagnostics

	id := serviceID + "/" + fmt.Sprintf("%d", version) + "/" + fastly.ToValue(ls.Name)

	model := Model{
		NestedModel: FlattenToNestedModel(ls),
