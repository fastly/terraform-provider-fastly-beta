...
func FlattenToNestedModel(l *fastly.Logshuttle) NestedModel {
	m := NestedModel{}
	...
	m.URL = types.StringValue(fastly.ToValue(l.URL))
	...
	m.Format = service.StringPointerOrDefault(l.Format, constants.LoggingLogshuttleDefaultFormat)
...
func ResetVCLOnlyToDefaults(m *NestedModel) {
	m.Format = types.StringValue(constants.LoggingLogshuttleDefaultFormat)
...
func FlattenToComputeNestedModel(l *fastly.Logshuttle) ComputeNestedModel {
	return ComputeNestedModel{commonModel: FlattenToNestedModel(l).commonModel}
}

func flatten(ctx context.Context, l *fastly.Logshuttle, m *Model) {
	if l == nil {
		tflog.Warn(ctx, "flatten called with nil Log Shuttle logging endpoint")
		return
	}
	...
	tflog.Debug(ctx, "Flattened Log Shuttle logging endpoint state", map[string]any{
		"id":      id,
