...
func defaultNestedModel() NestedModel {
	return NestedModel{
		...
		Format:            types.StringValue(constants.LoggingLogshuttleDefaultFormat),
...
func fullNestedModel() NestedModel {
	...
	m.Name = types.StringValue("test-logshuttle")
	m.URL = types.StringValue("https://east.logplex.io/logs")
	m.Authentication = NewAuthenticationObject(types.StringValue("logshuttle-token"))
	...
	tests := []struct {
		...
		},
		{
			...
			api: &fastly.Logshuttle{
				...
				URL:   new("https://east.logplex.io/logs"),
				Token: new("logshuttle-token"),
			},
...
func TestFlatten(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, m *Model) {
				assert.Equal(t, types.StringValue("service-123-5-test-logshuttle"), m.ID)
				assert.Equal(t, types.StringValue("service-123"), m.Service)
				assert.Equal(t, types.Int64Value(5), m.Version)
				assert.Equal(t, types.StringValue("test-logshuttle"), m.Name)
			},
		},
	}
...
func TestBuildCreateInput(t *testing.T) {
	tests := []struct {
		...
		version   int
		model     NestedModel
		validate  func(t *testing.T, input *fastly.CreateLogshuttleInput)
	}{
		{
			name:      "minimal model",
			...
			validate: func(t *testing.T, input *fastly.CreateLogshuttleInput) {
				...
				assert.Equal(t, "test-logshuttle", *input.Name)
				assert.Equal(t, "https://east.logplex.io/logs", *input.URL)
				assert.Equal(t, "logshuttle-token", *input.Token)
				assert.Equal(t, "none", *input.ProcessingRegion)
				assert.Equal(t, constants.LoggingLogshuttleDefaultFormat, *input.Format)
...
func TestBuildUpdateInput(t *testing.T) {
	...
	assert.Equal(t, 10, input.ServiceVersion)
	assert.Equal(t, "test-logshuttle", input.Name)
	assert.Equal(t, "test-logshuttle", *input.NewName)
	assert.Equal(t, "https://east.logplex.io/logs", *input.URL)
	assert.Equal(t, "logshuttle-token", *input.Token)
	assert.Equal(t, "eu", *input.ProcessingRegion)
	assert.Equal(t, "%h %l %u", *input.Format)
...
func TestClearVCLOnlyCreateFields(t *testing.T) {
	input := &fastly.CreateLogshuttleInput{
...
func TestResetVCLOnlyToDefaults(t *testing.T) {
	...
	m := FlattenToNestedModel(&fastly.Logshuttle{
		Name:              new("test-logshuttle"),
		URL:               new("https://east.logplex.io/logs"),
	...
	assert.Equal(t, constants.LoggingLogshuttleDefaultFormat, m.Format.ValueString())
	...
	assert.Equal(t, "test-logshuttle", m.Name.ValueString())
	assert.Equal(t, "https://east.logplex.io/logs", m.URL.ValueString())
	...
	tests := []struct {
		...
		},
		{
			...
			a: func() NestedModel {
				...
				m.URL = types.StringValue("https://one.example.com/logs")
				return m
			}(),
			b: func() NestedModel {
				m := minimalNestedModel()
				m.URL = types.StringValue("https://two.example.com/logs")
				return m
			}(),
			expected: false,
		},
...
// `logging_logshuttle { format = ... }` inside fastly_service_compute_auto
...
func TestComputeAttributesOmitsVCLOnly(t *testing.T) {
	...
	for _, name := range []string{"name", "url", "authentication", "processing_region"} {
...
func TestAuthenticationAttribute(t *testing.T) {
	...
	assert.True(t, auth.Required, "authentication is Required: there is no FASTLY_LOGSHUTTLE_* env var to default it from")
	...
	assert.True(t, token.Sensitive, "the Log Shuttle token must never be rendered in plan output")

	assert.Len(t, auth.Attributes, 1, "authentication holds only token for Log Shuttle")
}
...
func TestTokenAccessor(t *testing.T) {
	assert.Equal(t, "logshuttle-token", minimalNestedModel().Token().ValueString())
...
func TestSchemaValidators(t *testing.T) {
	...
	stringCases := []struct {
		...
		{"format rejects explicit empty string", "format", "", false},
	}
	...
	// name, url, token and response_condition accept any string; assert that
...
func TestValidateConditionReferences(t *testing.T) {
	...
	t.Run("references a condition that isn't configured", func(t *testing.T) {
		...
		if assert.Error(t, err) {
			assert.Contains(t, err.Error(), `"test-logshuttle"`)
