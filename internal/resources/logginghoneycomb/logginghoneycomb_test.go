...
func defaultNestedModel() NestedModel {
	return NestedModel{
		...
		Format:            types.StringValue(constants.LoggingHoneycombDefaultFormat),
...
func fullNestedModel() NestedModel {
	...
	m.Name = types.StringValue("test-honeycomb")
	m.Authentication = NewAuthenticationObject(types.StringValue("honeycomb-write-key"))
	m.Dataset = types.StringValue("fastly_honeycomb")
...
func TestBuildCreateInput(t *testing.T) {
	tests := []struct {
		...
		version   int
		model     NestedModel
		validate  func(t *testing.T, input *fastly.CreateHoneycombInput)
	}{
		{
			name:      "minimal model",
			...
			validate: func(t *testing.T, input *fastly.CreateHoneycombInput) {
				...
				assert.Equal(t, "test-honeycomb", *input.Name)
				assert.Equal(t, "honeycomb-write-key", *input.Token)
				assert.Equal(t, "fastly_honeycomb", *input.Dataset)
				assert.Equal(t, "none", *input.ProcessingRegion)
				assert.Equal(t, constants.LoggingHoneycombDefaultFormat, *input.Format)
...
func TestBuildUpdateInput(t *testing.T) {
	...
	assert.Equal(t, 10, input.ServiceVersion)
	assert.Equal(t, "test-honeycomb", input.Name)
	assert.Equal(t, "test-honeycomb", *input.NewName)
	assert.Equal(t, "honeycomb-write-key", *input.Token)
	assert.Equal(t, "fastly_honeycomb", *input.Dataset)
	assert.Equal(t, "eu", *input.ProcessingRegion)
	assert.Equal(t, "%h %l %u", *input.Format)
...
func TestClearVCLOnlyCreateFields(t *testing.T) {
	input := &fastly.CreateHoneycombInput{
...
func TestResetVCLOnlyToDefaults(t *testing.T) {
	...
	m := FlattenToNestedModel(&fastly.Honeycomb{
		Name:              new("test-honeycomb"),
		Token:             new("honeycomb-write-key"),
		Dataset:           new("fastly_honeycomb"),
	...
	assert.Equal(t, constants.LoggingHoneycombDefaultFormat, m.Format.ValueString())
	...
	assert.Equal(t, "test-honeycomb", m.Name.ValueString())
	assert.Equal(t, "honeycomb-write-key", m.Token().ValueString())
	assert.Equal(t, "fastly_honeycomb", m.Dataset.ValueString())
...
func TestModelsEqual(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			a: func() NestedModel {
				m := minimalNestedModel()
				m.Dataset = types.StringValue("dataset-a")
				return m
			}(),
			b: func() NestedModel {
				m := minimalNestedModel()
				m.Dataset = types.StringValue("dataset-b")
				return m
			}(),
			expected: false,
		},
...
func TestComputeModelsEqual(t *testing.T) {
	...
	b.Dataset = types.StringValue("different-dataset")
		...
		},
		{
			...
			b: []NestedModel{
				func() NestedModel {
					...
					m.Dataset = types.StringValue("other-dataset")
					return m
				}(),
			},
			expected: false,
...
func TestComputeEqual(t *testing.T) {
	...
	b[0].Dataset = types.StringValue("other-dataset")
...
// `logging_honeycomb { format = ... }` inside fastly_service_compute_auto fail
...
func TestComputeAttributesOmitsVCLOnly(t *testing.T) {
	...
	for _, name := range []string{"name", "authentication", "dataset", "processing_region"} {
...
func TestAuthenticationAttribute(t *testing.T) {
	...
	assert.True(t, auth.Required, "authentication is Required: there is no FASTLY_HONEYCOMB_* env var to default it from")
	...
	assert.True(t, token.Sensitive, "the Honeycomb write key must never be rendered in plan output")

	assert.Len(t, auth.Attributes, 1, "authentication holds only token for Honeycomb")
}
...
func TestTokenAccessor(t *testing.T) {
	assert.Equal(t, "honeycomb-write-key", minimalNestedModel().Token().ValueString())
...
func TestSchemaValidators(t *testing.T) {
	...
	// name, dataset, token, and response_condition accept any string; assert
	// that rather than leaving it implicit.
	assert.Empty(t, attrs["name"].(schema.StringAttribute).Validators)
	assert.Empty(t, attrs["dataset"].(schema.StringAttribute).Validators)
...
func TestValidateConditionReferences(t *testing.T) {
	...
	t.Run("references a condition that isn't configured", func(t *testing.T) {
		...
		if assert.Error(t, err) {
			assert.Contains(t, err.Error(), `"test-honeycomb"`)
