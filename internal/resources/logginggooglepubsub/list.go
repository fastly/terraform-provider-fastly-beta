...
func (l *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "List all Pub/Sub logging endpoints across all Fastly CDN and Compute services at their active version, or latest version when no active version exists.",
...
func (l *ListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	tflog.Debug(ctx, "Listing Fastly Pub/Sub logging endpoints")
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, svc := range services {
			...
			loggers, err := l.client.ListPubsubs(ctx, &fastly.ListPubsubsInput{
				ServiceID:      serviceID,
				ServiceVersion: version,
			})
			if err != nil {
				tflog.Warn(ctx, "Error listing Pub/Sub logging endpoints for service", map[string]any{
					"service_id": serviceID,
					"error":      err.Error(),
				})
				continue
			...
			for _, p := range loggers {
				...
				result.Diagnostics.Append(result.Identity.Set(ctx, &IdentityModel{
					...
					Name:      types.StringValue(fastly.ToValue(p.Name)),
				})...)
...
// accept.
func setResourceAttrs(ctx context.Context, result *list.ListResult, p *fastly.Pubsub, serviceID string, version int, serviceType string) diag.Diagnostics {
