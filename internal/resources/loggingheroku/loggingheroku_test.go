...
func defaultNestedModel() NestedModel {
	return NestedModel{
		...
		Format:            types.StringValue(constants.LoggingHerokuDefaultFormat),
...
func fullNestedModel() NestedModel {
	...
	m.Name = types.StringValue("test-heroku")
	m.URL = types.StringValue("https://logs.example.com")
	m.Authentication = NewAuthenticationObject(types.StringValue("heroku-token"))
...
func TestFlatten(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, m *Model) {
				assert.Equal(t, types.StringValue("service-123/5/test-heroku"), m.ID)
				assert.Equal(t, types.StringValue("service-123"), m.Service)
				assert.Equal(t, types.Int64Value(5), m.Version)
				assert.Equal(t, types.StringValue("test-heroku"), m.Name)
			},
		},
	}
...
func TestBuildCreateInput(t *testing.T) {
	tests := []struct {
		...
		version   int
		model     NestedModel
		validate  func(t *testing.T, input *fastly.CreateHerokuInput)
	}{
		{
			name:      "minimal model",
			...
			validate: func(t *testing.T, input *fastly.CreateHerokuInput) {
				...
				assert.Equal(t, "test-heroku", *input.Name)
				assert.Equal(t, "https://logs.example.com", *input.URL)
				assert.Equal(t, "heroku-token", *input.Token)
				assert.Equal(t, "none", *input.ProcessingRegion)
				assert.Equal(t, constants.LoggingHerokuDefaultFormat, *input.Format)
...
func TestBuildUpdateInput(t *testing.T) {
	...
	assert.Equal(t, 10, input.ServiceVersion)
	assert.Equal(t, "test-heroku", input.Name)
	assert.Equal(t, "test-heroku", *input.NewName)
	assert.Equal(t, "https://logs.example.com", *input.URL)
	assert.Equal(t, "heroku-token", *input.Token)
	assert.Equal(t, "eu", *input.ProcessingRegion)
	assert.Equal(t, "%h %l %u", *input.Format)
...
func TestResetVCLOnlyToDefaults(t *testing.T) {
	...
	m := FlattenToNestedModel(&fastly.Heroku{
		Name:              new("test-heroku"),
		URL:               new("https://logs.example.com"),
		Token:             new("heroku-token"),
	...
	assert.Equal(t, constants.LoggingHerokuDefaultFormat, m.Format.ValueString())
	...
	assert.Equal(t, "test-heroku", m.Name.ValueString())
	assert.Equal(t, "https://logs.example.com", m.URL.ValueString())
	assert.Equal(t, "heroku-token", m.Token().ValueString())
...
func TestComputeModelsEqual(t *testing.T) {
	...
	b.URL = types.StringValue("https://different.example.com")
		...
		},
		{
			...
			b: []NestedModel{
				func() NestedModel {
					...
					m.URL = types.StringValue("https://different.example.com")
					return m
				}(),
			},
			expected: false,
...
func TestComputeEqual(t *testing.T) {
	...
	b[0].URL = types.StringValue("https://different.example.com")
...
// `logging_heroku { format = ... }` inside fastly_service_compute_auto fail
...
func TestAuthenticationAttribute(t *testing.T) {
	...
	assert.True(t, auth.Required, "authentication is Required: there is no FASTLY_HEROKU_* env var to default it from")
	...
	assert.True(t, token.Sensitive, "the Heroku token must never be rendered in plan output")

	assert.Len(t, auth.Attributes, 1, "authentication holds only token for Heroku")
}
...
func TestTokenAccessor(t *testing.T) {
	assert.Equal(t, "heroku-token", minimalNestedModel().Token().ValueString())
...
func TestValidateConditionReferences(t *testing.T) {
	...
	t.Run("references a condition that isn't configured", func(t *testing.T) {
		...
		if assert.Error(t, err) {
			assert.Contains(t, err.Error(), `"test-heroku"`)
