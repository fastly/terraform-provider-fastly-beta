...
// loggingdigitalocean_test.go), but for the list resource: the API still
...
func TestSetResourceAttrs_computeResetsVCLOnlyFields(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantFormat:        types.StringValue(constants.LoggingDigitalOceanDefaultFormat),
