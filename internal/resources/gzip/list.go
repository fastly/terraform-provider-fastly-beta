...
func (l *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "List all gzip configurations across all Fastly CDN services at their active version, or latest version when no active version exists.",
...
func (l *ListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	tflog.Debug(ctx, "Listing Fastly service gzip configurations")
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, svc := range services {
			...
			gzips, err := l.client.ListGzips(ctx, &fastly.ListGzipsInput{
				ServiceID:      serviceID,
				ServiceVersion: version,
			})
			if err != nil {
				tflog.Warn(ctx, "Error listing gzip configurations for service", map[string]any{
					"service_id": serviceID,
					"error":      err.Error(),
				})
				continue
			...
			for _, g := range gzips {
				...
				if req.IncludeResource {
					result.Diagnostics.Append(setResourceAttrs(ctx, &result, g, serviceID, version)...)
				}
...
func setResourceAttrs(ctx context.Context, result *list.ListResult, g *fastly.Gzip, serviceID string, version int) diag.Diagnostics {
	var diags diag.Diagnostics

	id := serviceID + "-" + fmt.Sprintf("%d", version) + "-" + fastly.ToValue(g.Name)

	model := Model{
		NestedModel: ops{}.ToModel(g),
