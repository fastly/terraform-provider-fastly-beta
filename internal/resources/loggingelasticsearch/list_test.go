...
// normalization as TestResetVCLOnlyToDefaults, but for the list resource.
func TestSetResourceAttrs_computeResetsVCLOnlyFields(t *testing.T) {
	ctx := context.Background()

	es := &fastly.Elasticsearch{
	...
	tests := []struct {
		...
		},
		{
			...
			wantFormat:        types.StringValue(constants.LoggingElasticsearchDefaultFormat),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			diags := setResourceAttrs(ctx, &result, es, "service-id", 1, tt.serviceType)
