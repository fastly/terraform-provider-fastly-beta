...
func (l *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "List all Elasticsearch logging endpoints across all Fastly CDN and Compute services at their active version, or latest version when no active version exists.",
...
func (l *ListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	tflog.Debug(ctx, "Listing Fastly Elasticsearch logging endpoints")
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, svc := range services {
			...
			loggers, err := l.client.ListElasticsearch(ctx, &fastly.ListElasticsearchInput{
				ServiceID:      serviceID,
				ServiceVersion: version,
			})
			if err != nil {
				tflog.Warn(ctx, "Error listing Elasticsearch logging endpoints for service", map[string]any{
					"service_id": serviceID,
					"error":      err.Error(),
				})
				continue
			...
			for _, es := range loggers {
				if es == nil || es.Name == nil {
					continue
				}
				...
				result.Diagnostics.Append(result.Identity.Set(ctx, &IdentityModel{
					...
					Name:      types.StringValue(fastly.ToValue(es.Name)),
				})...)
...
// accept.
func setResourceAttrs(ctx context.Context, result *list.ListResult, es *fastly.Elasticsearch, serviceID string, version int, serviceType string) diag.Diagnostics {
	var diags diag.Diagnostics

	id := serviceID + "/" + fmt.Sprintf("%d", version) + "/" + fastly.ToValue(es.Name)

	model := Model{
		NestedModel: FlattenToNestedModel(es),
