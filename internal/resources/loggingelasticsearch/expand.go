...
func buildCommonCreateInput(serviceID string, version int, m commonModel) *fastly.CreateElasticsearchInput {
	input := &fastly.CreateElasticsearchInput{
		ServiceID:      serviceID,
		ServiceVersion: version,
		Name:           new(service.StringValue(m.Name)),
		Index:          new(service.StringValue(m.Index)),
	...
	input.Pipeline = fastly.NullString(service.StringValue(m.Pipeline))
...
func BuildCreateInput(serviceID string, version int, m NestedModel) *fastly.CreateElasticsearchInput {
...
func BuildComputeCreateInput(serviceID string, version int, m ComputeNestedModel) *fastly.CreateElasticsearchInput {
...
// service-type-specific fields on top of this.
func buildCommonUpdateInput(serviceID string, version int, m commonModel) *fastly.UpdateElasticsearchInput {
	...
	// user/password/tls.*/pipeline default to "" rather than being
	// omitted-when-empty, so they must always be sent as a concrete value on
	// update — otherwise clearing one back to "" would be silently dropped
	// (fastly.NullString maps "" to nil, which omits the field and leaves a
	...
	input.Pipeline = new(service.StringValue(m.Pipeline))
	input.ProcessingRegion = fastly.NullString(service.StringValue(m.ProcessingRegion))
	...
	// so — like the fields above — they must always be sent rather than omitted
	// when zero (fastly.NullInt maps 0 to nil).
...
func BuildUpdateInput(serviceID string, version int, m NestedModel) *fastly.UpdateElasticsearchInput {
	...
	// schema.go). UpdateElasticsearchInput.Placement is a *Nullable[string]
...
// sets format, format_version, placement, or response_condition, since those
// only affect generated VCL and Compute services don't have any.
func BuildComputeUpdateInput(serviceID string, version int, m ComputeNestedModel) *fastly.UpdateElasticsearchInput {
...
// response_condition on a CreateElasticsearchInput. The standalone
// fastly_service_logging_elasticsearch resource shares one schema across both
...
func ClearVCLOnlyCreateFields(input *fastly.CreateElasticsearchInput) {
	input.Format = nil
...
// UpdateElasticsearchInput.
func ClearVCLOnlyUpdateFields(input *fastly.UpdateElasticsearchInput) {
	input.Format = nil
	input.FormatVersion = nil
	input.Placement = nil
	input.ResponseCondition = nil
