...
func defaultNestedModel() NestedModel {
	return NestedModel{
		...
		Format:            types.StringValue(constants.LoggingFTPDefaultFormat),
...
func fullNestedModel() NestedModel {
	...
	m.Name = types.StringValue("test-ftp")
	m.Address = types.StringValue("ftp.example.com")
...
func TestFlatten(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, m *Model) {
				assert.Equal(t, types.StringValue("service-123-5-test-ftp"), m.ID)
				assert.Equal(t, types.StringValue("service-123"), m.Service)
				assert.Equal(t, types.Int64Value(5), m.Version)
				assert.Equal(t, types.StringValue("test-ftp"), m.Name)
			},
		},
	}
...
func TestBuildCreateInput(t *testing.T) {
	tests := []struct {
		...
		version   int
		model     NestedModel
		validate  func(t *testing.T, input *fastly.CreateFTPInput)
	}{
		{
			name:      "minimal model",
			...
			validate: func(t *testing.T, input *fastly.CreateFTPInput) {
				...
				assert.Equal(t, "test-ftp", *input.Name)
				assert.Equal(t, "ftp.example.com", *input.Address)
				...
				assert.Equal(t, constants.LoggingFTPDefaultFormat, *input.Format)
		...
		},
		{
			...
			validate: func(t *testing.T, input *fastly.CreateFTPInput) {
				...
				assert.Equal(t, 2121, *input.Port)
...
func TestBuildUpdateInput(t *testing.T) {
	...
	assert.Equal(t, 10, input.ServiceVersion)
	assert.Equal(t, "test-ftp", input.Name)
	assert.Equal(t, "test-ftp", *input.NewName)
	assert.Equal(t, "ftp.example.com", *input.Address)
	assert.Equal(t, "test-user", *input.Username)
	assert.Equal(t, "test-password", *input.Password)
	assert.Equal(t, "/logs/", *input.Path)
...
func TestResetVCLOnlyToDefaults(t *testing.T) {
	...
	m := FlattenToNestedModel(&fastly.FTP{
		Name:              new("test-ftp"),
		Address:           new("ftp.example.com"),
		Username:          new("test-user"),
		Password:          new("test-password"),
	...
	assert.Equal(t, constants.LoggingFTPDefaultFormat, m.Format.ValueString())
	...
	assert.Equal(t, "test-ftp", m.Name.ValueString())
...
// TestUserPasswordAccessors covers the object-unwrapping accessors, including
...
func TestUserPasswordAccessors(t *testing.T) {
...
func TestModelsEqual(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			a: func() NestedModel {
				m := minimalNestedModel()
				m.Authentication = NewAuthenticationObject(types.StringValue("test-user"), types.StringValue("pw-1"))
				return m
			}(),
			b: func() NestedModel {
				m := minimalNestedModel()
				m.Authentication = NewAuthenticationObject(types.StringValue("test-user"), types.StringValue("pw-2"))
				return m
			}(),
			expected: false,
		},
...
// `logging_ftp { format = ... }` inside fastly_service_compute_auto fail at
...
func TestComputeAttributesOmitsVCLOnly(t *testing.T) {
	...
	for _, name := range []string{"name", "address", "authentication", "path", "processing_region"} {
		assert.Contains(t, compute, name)
		assert.Contains(t, common, name)
	}

	// user/password are nested under authentication, never top-level attributes.
	assert.NotContains(t, compute, "user")
	assert.NotContains(t, compute, "password")
	assert.NotContains(t, common, "user")
	assert.NotContains(t, common, "password")
}
...
// `password` inside, matching how every other logging endpoint groups
// credentials.
func TestAuthenticationAttribute(t *testing.T) {
	...
	assert.True(t, auth.Required, "authentication is Required: there is no FASTLY_FTP_* environment variable to default it from")
	...
	assert.True(t, password.Sensitive, "the FTP password must never be rendered in plan output")

	assert.Len(t, auth.Attributes, 2, "authentication holds user and password for FTP")
}
...
func TestSchemaValidators(t *testing.T) {
	...
	// name, address, path, and response_condition accept any string; assert
...
func TestValidateConditionReferences(t *testing.T) {
	...
	t.Run("references a condition that isn't configured", func(t *testing.T) {
		...
		if assert.Error(t, err) {
			assert.Contains(t, err.Error(), `"test-ftp"`)
