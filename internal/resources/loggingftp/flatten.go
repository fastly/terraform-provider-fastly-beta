...
func FlattenToNestedModel(f *fastly.FTP) NestedModel {
	m := NestedModel{}
	...
	m.Authentication = NewAuthenticationObject(
		service.StringPointerOrDefault(f.Username, ""),
		service.StringPointerOrDefault(f.Password, ""),
	)
	...
	m.Format = service.StringPointerOrDefault(f.Format, constants.LoggingFTPDefaultFormat)
...
func ResetVCLOnlyToDefaults(m *NestedModel) {
	m.Format = types.StringValue(constants.LoggingFTPDefaultFormat)
...
func FlattenToComputeNestedModel(f *fastly.FTP) ComputeNestedModel {
...
func flatten(ctx context.Context, f *fastly.FTP, m *Model) {
	if f == nil {
		tflog.Warn(ctx, "flatten called with nil FTP logging endpoint")
		return
	}
	...
	tflog.Debug(ctx, "Flattened FTP logging endpoint state", map[string]any{
		"id":      id,
