...
func defaultNestedModel() NestedModel {
	return NestedModel{
		...
		Format:            types.StringValue(constants.LoggingGrafanaCloudLogsDefaultFormat),
...
func fullNestedModel() NestedModel {
	...
	m.Name = types.StringValue("test-grafanacloudlogs")
	m.URL = types.StringValue("https://test123.grafana.net")
	m.User = types.StringValue("123456")
	m.Index = types.StringValue(`{"label": "value"}`)
	m.Authentication = NewAuthenticationObject(types.StringValue("grafanacloudlogs-token"))
	...
	tests := []struct {
		...
		},
		{
			...
			api: &fastly.GrafanaCloudLogs{
				...
				Token: new("grafanacloudlogs-token"),
			},
...
func TestFlatten(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, m *Model) {
				assert.Equal(t, types.StringValue("service-123-5-test-grafanacloudlogs"), m.ID)
				assert.Equal(t, types.StringValue("service-123"), m.Service)
				assert.Equal(t, types.Int64Value(5), m.Version)
				assert.Equal(t, types.StringValue("test-grafanacloudlogs"), m.Name)
			},
		},
	}
...
func TestBuildCreateInput(t *testing.T) {
	tests := []struct {
		...
		version   int
		model     NestedModel
		validate  func(t *testing.T, input *fastly.CreateGrafanaCloudLogsInput)
	}{
		{
			name:      "minimal model",
			...
			validate: func(t *testing.T, input *fastly.CreateGrafanaCloudLogsInput) {
				...
				assert.Equal(t, "test-grafanacloudlogs", *input.Name)
				assert.Equal(t, "https://test123.grafana.net", *input.URL)
				assert.Equal(t, "123456", *input.User)
				assert.Equal(t, `{"label": "value"}`, *input.Index)
				assert.Equal(t, "grafanacloudlogs-token", *input.Token)
				assert.Equal(t, "none", *input.ProcessingRegion)
				assert.Equal(t, constants.LoggingGrafanaCloudLogsDefaultFormat, *input.Format)
...
func TestBuildUpdateInput(t *testing.T) {
	...
	assert.Equal(t, 10, input.ServiceVersion)
	assert.Equal(t, "test-grafanacloudlogs", input.Name)
	assert.Equal(t, "test-grafanacloudlogs", *input.NewName)
	assert.Equal(t, "https://test123.grafana.net", *input.URL)
	assert.Equal(t, "123456", *input.User)
	assert.Equal(t, `{"label": "value"}`, *input.Index)
	assert.Equal(t, "grafanacloudlogs-token", *input.Token)
...
func TestResetVCLOnlyToDefaults(t *testing.T) {
	...
	m := FlattenToNestedModel(&fastly.GrafanaCloudLogs{
		Name:              new("test-grafanacloudlogs"),
		URL:               new("https://test123.grafana.net"),
		User:              new("123456"),
		Index:             new(`{"label": "value"}`),
	...
	assert.Equal(t, constants.LoggingGrafanaCloudLogsDefaultFormat, m.Format.ValueString())
	...
	assert.Equal(t, "test-grafanacloudlogs", m.Name.ValueString())
	assert.Equal(t, "https://test123.grafana.net", m.URL.ValueString())
	assert.Equal(t, "123456", m.User.ValueString())
	assert.Equal(t, `{"label": "value"}`, m.Index.ValueString())
	assert.Equal(t, "grafanacloudlogs-token", m.Token().ValueString())
...
func TestComputeModelsEqual(t *testing.T) {
	...
	b.User = types.StringValue("999999")
...
func TestComputeModelsEqualIgnoresVCLOnlyFields(t *testing.T) {
	...
	remote := &fastly.GrafanaCloudLogs{
		Name:              new("test-grafanacloudlogs"),
		URL:               new("https://test123.grafana.net"),
		User:              new("123456"),
		Index:             new(`{"label": "value"}`),
		...
		},
		{
			...
			b: []NestedModel{
				func() NestedModel {
					...
					m.User = types.StringValue("999999")
					return m
				}(),
			},
			expected: false,
...
func TestComputeEqual(t *testing.T) {
	...
	b[0].User = types.StringValue("999999")
...
// `logging_grafanacloudlogs { format = ... }` inside fastly_service_compute_auto
...
func TestComputeAttributesOmitsVCLOnly(t *testing.T) {
	...
	for _, name := range []string{"name", "url", "user", "index", "authentication", "processing_region"} {
...
func TestAuthenticationAttribute(t *testing.T) {
	...
	assert.True(t, auth.Required, "authentication is Required: there is no FASTLY_GRAFANACLOUDLOGS_* env var to default it from")
	...
	assert.True(t, token.Sensitive, "the Grafana Cloud Logs access policy token must never be rendered in plan output")

	assert.Len(t, auth.Attributes, 1, "authentication holds only token for Grafana Cloud Logs")
}
...
func TestTokenAccessor(t *testing.T) {
	assert.Equal(t, "grafanacloudlogs-token", minimalNestedModel().Token().ValueString())
...
// "none" only, format capped at 12288 and rejecting an explicit "" (see
// validation.NotBlank — an explicit empty string bypasses the schema Default
// and is silently dropped by the Fastly API, which produces "Provider
// produced inconsistent result after apply" rather than a plan-time error).
func TestSchemaValidators(t *testing.T) {
	...
	stringCases := []struct {
		...
		{"format rejects empty", "format", "", false},
	}
	...
	// name, url, user, index, token and response_condition accept any string;
	...
	assert.Empty(t, attrs["user"].(schema.StringAttribute).Validators)
...
func TestValidateConditionReferences(t *testing.T) {
	...
	t.Run("references a condition that isn't configured", func(t *testing.T) {
		...
		if assert.Error(t, err) {
			assert.Contains(t, err.Error(), `"test-grafanacloudlogs"`)
