...
func TestValidItemsValidatorEnforcesMaximumDictionarySize(t *testing.T) {
	tests := []struct {
		name      string
		size      int
		wantError bool
	}{
		{name: "at limit", size: fastly.MaximumDictionarySize},
		{name: "over limit", size: fastly.MaximumDictionarySize + 1, wantError: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			items := make(map[string]attr.Value, tt.size)
			for i := range tt.size {
				items[fmt.Sprintf("key-%d", i)] = types.StringValue("value")
			}

			value, valueDiags := types.MapValue(types.StringType, items)
			require.False(t, valueDiags.HasError())
