...
func defaultNestedModel() NestedModel {
	return NestedModel{
		...
		Format:            types.StringValue(constants.LoggingKinesisDefaultFormat),
...
func defaultCommonModel() commonModel {
	return commonModel{
		...
		Authentication:   NewAuthenticationObject(types.StringValue(DefaultAccessKey), types.StringValue(DefaultSecretKey), types.StringValue(DefaultIAMRole)),
...
func fullNestedModel() NestedModel {
	...
	m.Name = types.StringValue("test-kinesis")
	m.Topic = types.StringValue("test-stream")
	m.Region = types.StringValue("us-west-2")
	m.Authentication = NewAuthenticationObject(types.StringValue("access-key"), types.StringValue("secret-key"), types.StringValue(""))
...
func TestFlatten(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, m *Model) {
				assert.Equal(t, types.StringValue("service-123-5-test-kinesis"), m.ID)
				assert.Equal(t, types.StringValue("service-123"), m.Service)
				assert.Equal(t, types.Int64Value(5), m.Version)
				assert.Equal(t, types.StringValue("test-kinesis"), m.Name)
			},
		},
	}
...
func TestBuildCreateInput(t *testing.T) {
	tests := []struct {
		...
		version   int
		model     NestedModel
		validate  func(t *testing.T, input *fastly.CreateKinesisInput)
	}{
		{
			name:      "minimal model",
			...
			validate: func(t *testing.T, input *fastly.CreateKinesisInput) {
				...
				assert.Equal(t, "test-kinesis", *input.Name)
				...
				assert.Equal(t, constants.LoggingKinesisDefaultFormat, *input.Format)
				assert.Nil(t, input.AccessKey, "empty access_key must be omitted, not sent as \"\"")
				assert.Nil(t, input.SecretKey, "empty secret_key must be omitted, not sent as \"\"")
				assert.Nil(t, input.IAMRole, "empty iam_role must be omitted, not sent as \"\"")
...
func TestBuildUpdateInput(t *testing.T) {
	...
	assert.Equal(t, 10, input.ServiceVersion)
	assert.Equal(t, "test-kinesis", input.Name)
	assert.Equal(t, "test-kinesis", *input.NewName)
	assert.Equal(t, "test-stream", *input.StreamName)
	assert.Equal(t, "us-west-2", *input.Region)
	assert.Equal(t, "access-key", *input.AccessKey)
	assert.Equal(t, "secret-key", *input.SecretKey)
...
func TestBuildUpdateInputClearsClearableFields(t *testing.T) {
	...
	assert.NotNil(t, input.AccessKey, "access_key must be sent even when empty")
	assert.Equal(t, "", *input.AccessKey)
	assert.NotNil(t, input.SecretKey, "secret_key must be sent even when empty")
	assert.Equal(t, "", *input.SecretKey)
	assert.NotNil(t, input.IAMRole, "iam_role must be sent even when empty")
	assert.Equal(t, "", *input.IAMRole)
...
func TestResetVCLOnlyToDefaults(t *testing.T) {
	...
	m := FlattenToNestedModel(&fastly.Kinesis{
		Name:              new("test-kinesis"),
		StreamName:        new("test-stream"),
	...
	assert.Equal(t, constants.LoggingKinesisDefaultFormat, m.Format.ValueString())
	...
	assert.Equal(t, "test-kinesis", m.Name.ValueString())
	assert.Equal(t, "test-stream", m.Topic.ValueString())
	...
	tests := []struct {
		...
		},
		{
			...
			a: func() NestedModel {
				...
				m.Topic = types.StringValue("stream-one")
				return m
			}(),
			b: func() NestedModel {
				m := minimalNestedModel()
				m.Topic = types.StringValue("stream-two")
				return m
			}(),
			expected: false,
		},
		{
			...
			a: func() NestedModel {
				m := minimalNestedModel()
				m.Authentication = NewAuthenticationObject(types.StringValue(""), types.StringValue("secret-1"), types.StringValue(""))
				return m
			}(),
			b: func() NestedModel {
				m := minimalNestedModel()
				m.Authentication = NewAuthenticationObject(types.StringValue(""), types.StringValue("secret-2"), types.StringValue(""))
				return m
			}(),
			expected: false,
		},
		{
			name: "different iam_role",
			a: func() NestedModel {
				m := minimalNestedModel()
				m.Authentication = NewAuthenticationObject(types.StringValue(""), types.StringValue(""), types.StringValue("role-1"))
				return m
			}(),
			b: func() NestedModel {
				m := minimalNestedModel()
				m.Authentication = NewAuthenticationObject(types.StringValue(""), types.StringValue(""), types.StringValue("role-2"))
				return m
			}(),
			expected: false,
		},
...
// `logging_kinesis { format = ... }` inside fastly_service_compute_auto fail at
...
func TestComputeAttributesOmitsVCLOnly(t *testing.T) {
	...
	for _, name := range []string{"name", "topic", "region", "authentication", "processing_region"} {
		assert.Contains(t, compute, name)
		assert.Contains(t, common, name)
	}

	// access_key/secret_key/iam_role are nested, never top-level attributes.
	for _, name := range []string{"access_key", "secret_key", "iam_role"} {
		assert.NotContains(t, compute, name)
...
// Computed `authentication` object with `access_key`/`secret_key`/`iam_role`
// inside, all defaulting to "" — the live (SDKv2) provider has no
// environment variable for any of the three credentials.
func TestAuthenticationAttribute(t *testing.T) {
	...
	assert.True(t, accessKey.Optional)
	assert.True(t, accessKey.Computed)
	assert.True(t, accessKey.Sensitive, "the AWS access key must never be rendered in plan output")
	...
	assert.True(t, secretKey.Optional)
	assert.True(t, secretKey.Computed)
	assert.True(t, secretKey.Sensitive, "the AWS secret key must never be rendered in plan output")

	iamRole, ok := auth.Attributes["iam_role"].(schema.StringAttribute)
	require.True(t, ok, "authentication.iam_role must be a StringAttribute")
	assert.True(t, iamRole.Optional)
	assert.True(t, iamRole.Computed)
	assert.False(t, iamRole.Sensitive, "an IAM role ARN is not itself credential material")

	assert.Len(t, auth.Attributes, 3, "authentication holds access_key, secret_key, and iam_role for Kinesis")
}
...
func TestCredentialAccessors(t *testing.T) {
	...
	assert.Equal(t, "access-key", full.AccessKey().ValueString())
	assert.Equal(t, "secret-key", full.SecretKey().ValueString())
	assert.Equal(t, "", full.IAMRole().ValueString())
	...
	tests := map[string]types.Object{
		...
		"null access_key":  NewAuthenticationObject(types.StringNull(), types.StringValue(""), types.StringValue("")),
		"unknown iam_role": NewAuthenticationObject(types.StringValue(""), types.StringValue(""), types.StringUnknown()),
	}
	for name, obj := range tests {
		t.Run(name, func(t *testing.T) {
			...
			assert.Equal(t, "", m.IAMRole().ValueString())
		})
	}
}
...
func TestSchemaValidators(t *testing.T) {
	...
	// name, topic, region, and response_condition accept any value; assert that
	...
	assert.Empty(t, attrs["region"].(schema.StringAttribute).Validators)
	...
	assert.Empty(t, auth.Attributes["access_key"].(schema.StringAttribute).Validators)
	assert.Empty(t, auth.Attributes["secret_key"].(schema.StringAttribute).Validators)
	assert.Empty(t, auth.Attributes["iam_role"].(schema.StringAttribute).Validators)
}
...
func TestValidateConditionReferences(t *testing.T) {
	...
	t.Run("references a condition that isn't configured", func(t *testing.T) {
		...
		if assert.Error(t, err) {
			assert.Contains(t, err.Error(), `"test-kinesis"`)
