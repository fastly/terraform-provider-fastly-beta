...
// normalization gap as TestResetVCLOnlyToDefaults (see loggingkinesis_test.go),
...
func TestSetResourceAttrs_computeResetsVCLOnlyFields(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantFormat:        types.StringValue(constants.LoggingKinesisDefaultFormat),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			diags := setResourceAttrs(ctx, &result, k, "service-id", 1, tt.serviceType)
