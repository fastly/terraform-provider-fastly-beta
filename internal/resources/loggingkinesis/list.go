...
func (l *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "List all Kinesis logging endpoints across all Fastly CDN and Compute services at their active version, or latest version when no active version exists.",
...
func (l *ListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	tflog.Debug(ctx, "Listing Fastly Kinesis logging endpoints")
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, svc := range services {
			...
			loggers, err := l.client.ListKinesis(ctx, &fastly.ListKinesisInput{
				ServiceID:      serviceID,
				ServiceVersion: version,
			})
			if err != nil {
				tflog.Warn(ctx, "Error listing Kinesis logging endpoints for service", map[string]any{
					"service_id": serviceID,
					"error":      err.Error(),
				})
				continue
			...
			for _, k := range loggers {
				...
				result.DisplayName = service.ToGeneratedResourceName(fastly.ToValue(svc.Name), serviceID, *k.Name)
				result.Diagnostics.Append(result.Identity.Set(ctx, &IdentityModel{
					ServiceID: types.StringValue(serviceID),
					Name:      types.StringValue(fastly.ToValue(k.Name)),
				})...)

				if req.IncludeResource {
					result.Diagnostics.Append(setResourceAttrs(ctx, &result, k, serviceID, version, fastly.ToValue(svc.Type))...)
				}
...
// accept.
func setResourceAttrs(ctx context.Context, result *list.ListResult, k *fastly.Kinesis, serviceID string, version int, serviceType string) diag.Diagnostics {
	var diags diag.Diagnostics

	id := serviceID + "/" + fmt.Sprintf("%d", version) + "/" + fastly.ToValue(k.Name)

	model := Model{
		NestedModel: FlattenToNestedModel(k),
