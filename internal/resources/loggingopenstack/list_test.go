...
// loggingopenstack_test.go), but for the list resource: the API still returns
// its own values for the VCL-only fields on a Compute-attached endpoint, and
// those must be reset to schema defaults here exactly as they are in
// Read/Import, or the list resource emits data the standalone resource's own
// schema rejects.
func TestSetResourceAttrs_computeResetsVCLOnlyFields(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantFormat:        types.StringValue(constants.LoggingOpenStackDefaultFormat),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			diags := setResourceAttrs(ctx, &result, o, "service-id", 1, tt.serviceType)
