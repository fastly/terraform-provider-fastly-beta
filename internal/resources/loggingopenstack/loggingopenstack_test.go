...
func defaultNestedModel() NestedModel {
	return NestedModel{
		...
		Format:            types.StringValue(constants.LoggingOpenStackDefaultFormat),
...
func fullNestedModel() NestedModel {
	...
	m.Name = types.StringValue("test-openstack")
	m.BucketName = types.StringValue("test-bucket")
	m.URL = types.StringValue("https://auth.storage.example.com/v1.0")
	...
	tests := []struct {
		...
		},
		{
			...
			api: &fastly.Openstack{
				...
				URL:        new("https://auth.storage.example.com/v1.0"),
				User:       new("test-user"),
...
func TestFlatten(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, m *Model) {
				assert.Equal(t, types.StringValue("service-123/5/test-openstack"), m.ID)
				assert.Equal(t, types.StringValue("service-123"), m.Service)
				assert.Equal(t, types.Int64Value(5), m.Version)
				assert.Equal(t, types.StringValue("test-openstack"), m.Name)
			},
		},
	}
...
func TestBuildCreateInput(t *testing.T) {
	tests := []struct {
		...
		version   int
		model     NestedModel
		validate  func(t *testing.T, input *fastly.CreateOpenstackInput)
	}{
		{
			name:      "minimal model",
			...
			validate: func(t *testing.T, input *fastly.CreateOpenstackInput) {
				...
				assert.Equal(t, "test-openstack", *input.Name)
				assert.Equal(t, "test-bucket", *input.BucketName)
				assert.Equal(t, "https://auth.storage.example.com/v1.0", *input.URL)
				...
				assert.Equal(t, constants.LoggingOpenStackDefaultFormat, *input.Format)
...
func TestBuildUpdateInput(t *testing.T) {
	...
	assert.Equal(t, 10, input.ServiceVersion)
	assert.Equal(t, "test-openstack", input.Name)
	assert.Equal(t, "test-openstack", *input.NewName)
	assert.Equal(t, "test-bucket", *input.BucketName)
	assert.Equal(t, "https://auth.storage.example.com/v1.0", *input.URL)
	assert.Equal(t, "test-user", *input.User)
	assert.Equal(t, "test-access-key", *input.AccessKey)
...
func TestClearVCLOnlyCreateFields(t *testing.T) {
	input := &fastly.CreateOpenstackInput{
...
func TestResetVCLOnlyToDefaults(t *testing.T) {
	...
	m := FlattenToNestedModel(&fastly.Openstack{
		Name:              new("test-openstack"),
		BucketName:        new("test-bucket"),
		URL:               new("https://auth.storage.example.com/v1.0"),
		User:              new("test-user"),
	...
	assert.Equal(t, constants.LoggingOpenStackDefaultFormat, m.Format.ValueString())
	...
	assert.Equal(t, "test-openstack", m.Name.ValueString())
...
func TestModelsEqual(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			a: func() NestedModel {
				m := minimalNestedModel()
				m.URL = types.StringValue("https://one.example.com/v1.0")
				return m
			}(),
			b: func() NestedModel {
				m := minimalNestedModel()
				m.URL = types.StringValue("https://two.example.com/v1.0")
				return m
			}(),
			expected: false,
		},
...
func TestComputeModelsEqual(t *testing.T) {
	...
	b.ProcessingRegion = types.StringValue("eu")
...
func TestComputeEqual(t *testing.T) {
	...
	b[0].ProcessingRegion = types.StringValue("eu")
...
// `logging_openstack { format = ... }` inside fastly_service_compute_auto fail
...
func TestComputeAttributesOmitsVCLOnly(t *testing.T) {
	...
	for _, name := range []string{"name", "bucket_name", "url", "authentication", "processing_region"} {
...
func TestAuthenticationAttribute(t *testing.T) {
	...
	assert.True(t, auth.Required, "authentication is Required: there is no FASTLY_OPENSTACK_* env var to default it from")
	...
	assert.True(t, accessKey.Sensitive, "the OpenStack access key must never be rendered in plan output")

	assert.Len(t, auth.Attributes, 2, "authentication holds user and access_key for OpenStack")
}
...
func TestSchemaValidators(t *testing.T) {
	...
	// name, url, response_condition, and bucket_name accept any string; assert
...
func TestValidateConditionReferences(t *testing.T) {
	...
	t.Run("references a condition that isn't configured", func(t *testing.T) {
		...
		if assert.Error(t, err) {
			assert.Contains(t, err.Error(), `"test-openstack"`)
