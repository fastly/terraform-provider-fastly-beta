...
func (l *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "List all dictionaries across all Fastly services at their active version, or latest version when no active version exists.",
...
func (l *ListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	tflog.Debug(ctx, "Listing Fastly service dictionaries")
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, svc := range services {
			...
			dicts, err := l.client.ListDictionaries(ctx, &fastly.ListDictionariesInput{
				ServiceID:      serviceID,
				ServiceVersion: version,
			})
			if err != nil {
				tflog.Warn(ctx, "Error listing dictionaries for service", map[string]any{
					"service_id": serviceID,
					"error":      err.Error(),
				})
				continue
...
func setResourceAttrs(ctx context.Context, result *list.ListResult, d *fastly.Dictionary, serviceID string, version int) diag.Diagnostics {
	var diags diag.Diagnostics

	id := serviceID + "-" + fmt.Sprintf("%d", version) + "-" + fastly.ToValue(d.Name)

	model := Model{
		NestedModel: ops{}.ToModel(d),
