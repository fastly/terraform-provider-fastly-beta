...
func defaultNestedModel() NestedModel {
	return NestedModel{
		...
		Format:            types.StringValue(constants.LoggingLogglyDefaultFormat),
...
func fullNestedModel() NestedModel {
	...
	m.Name = types.StringValue("test-loggly")
	m.Authentication = NewAuthenticationObject(types.StringValue("loggly-token"))
...
func TestBuildCreateInput(t *testing.T) {
	tests := []struct {
		...
		version   int
		model     NestedModel
		validate  func(t *testing.T, input *fastly.CreateLogglyInput)
	}{
		{
			name:      "minimal model",
			...
			validate: func(t *testing.T, input *fastly.CreateLogglyInput) {
				...
				assert.Equal(t, "test-loggly", *input.Name)
				assert.Equal(t, "loggly-token", *input.Token)
				assert.Equal(t, "none", *input.ProcessingRegion)
				assert.Equal(t, constants.LoggingLogglyDefaultFormat, *input.Format)
...
func TestBuildUpdateInput(t *testing.T) {
	...
	assert.Equal(t, 10, input.ServiceVersion)
	assert.Equal(t, "test-loggly", input.Name)
	assert.Equal(t, "test-loggly", *input.NewName)
	assert.Equal(t, "loggly-token", *input.Token)
	assert.Equal(t, "eu", *input.ProcessingRegion)
	assert.Equal(t, "%h %l %u", *input.Format)
	assert.Equal(t, 1, *input.FormatVersion)
...
func TestClearVCLOnlyCreateFields(t *testing.T) {
	input := &fastly.CreateLogglyInput{
...
func TestResetVCLOnlyToDefaults(t *testing.T) {
	...
	m := FlattenToNestedModel(&fastly.Loggly{
		Name:              new("test-loggly"),
		Token:             new("loggly-token"),
	...
	assert.Equal(t, constants.LoggingLogglyDefaultFormat, m.Format.ValueString())
	...
	assert.Equal(t, "test-loggly", m.Name.ValueString())
	assert.Equal(t, "loggly-token", m.Token().ValueString())
...
// `logging_loggly { format = ... }` inside fastly_service_compute_auto fail
...
func TestComputeAttributesOmitsVCLOnly(t *testing.T) {
	...
	for _, name := range []string{"name", "authentication", "processing_region"} {
...
func TestAuthenticationAttribute(t *testing.T) {
	...
	assert.True(t, auth.Required, "authentication is Required: there is no FASTLY_LOGGLY_* env var to default it from")
	...
	assert.True(t, token.Sensitive, "the Loggly token must never be rendered in plan output")

	assert.Len(t, auth.Attributes, 1, "authentication holds only token for Loggly")
}
...
func TestTokenAccessor(t *testing.T) {
	assert.Equal(t, "loggly-token", minimalNestedModel().Token().ValueString())
...
func TestValidateConditionReferences(t *testing.T) {
	...
	t.Run("references a condition that isn't configured", func(t *testing.T) {
		...
		if assert.Error(t, err) {
			assert.Contains(t, err.Error(), `"test-loggly"`)
