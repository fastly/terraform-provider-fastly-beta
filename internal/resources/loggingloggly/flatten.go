...
func FlattenToNestedModel(l *fastly.Loggly) NestedModel {
	m := NestedModel{}
	...
	m.Format = service.StringPointerOrDefault(l.Format, constants.LoggingLogglyDefaultFormat)
...
func ResetVCLOnlyToDefaults(m *NestedModel) {
	m.Format = types.StringValue(constants.LoggingLogglyDefaultFormat)
...
func FlattenToComputeNestedModel(l *fastly.Loggly) ComputeNestedModel {
	return ComputeNestedModel{commonModel: FlattenToNestedModel(l).commonModel}
}

func flatten(ctx context.Context, l *fastly.Loggly, m *Model) {
	if l == nil {
		tflog.Warn(ctx, "flatten called with nil Loggly logging endpoint")
		return
	}
	...
	tflog.Debug(ctx, "Flattened Loggly logging endpoint state", map[string]any{
		"id":      id,
