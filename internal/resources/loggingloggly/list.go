...
func (l *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "List all Loggly logging endpoints across all Fastly CDN and Compute services at their active version, or latest version when no active version exists.",
...
func (l *ListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	tflog.Debug(ctx, "Listing Fastly Loggly logging endpoints")
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, svc := range services {
			...
			loggers, err := l.client.ListLoggly(ctx, &fastly.ListLogglyInput{
				ServiceID:      serviceID,
				ServiceVersion: version,
			})
			if err != nil {
				tflog.Warn(ctx, "Error listing Loggly logging endpoints for service", map[string]any{
					"service_id": serviceID,
					"error":      err.Error(),
				})
				continue
			...
			for _, lg := range loggers {
				...
				result.DisplayName = service.ToGeneratedResourceName(fastly.ToValue(svc.Name), serviceID, *lg.Name)
				result.Diagnostics.Append(result.Identity.Set(ctx, &IdentityModel{
					ServiceID: types.StringValue(serviceID),
					Name:      types.StringValue(fastly.ToValue(lg.Name)),
				})...)

				if req.IncludeResource {
					result.Diagnostics.Append(setResourceAttrs(ctx, &result, lg, serviceID, version, fastly.ToValue(svc.Type))...)
				}
...
// accept.
func setResourceAttrs(ctx context.Context, result *list.ListResult, l *fastly.Loggly, serviceID string, version int, serviceType string) diag.Diagnostics {
	var diags diag.Diagnostics

	id := serviceID + "/" + fmt.Sprintf("%d", version) + "/" + fastly.ToValue(l.Name)

	model := Model{
		NestedModel: FlattenToNestedModel(l),
