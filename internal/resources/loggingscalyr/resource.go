...
func (r *Resource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_service_logging_scalyr"
}
	...
	resp.IdentitySchema = identityschema.Schema{
		Attributes: map[string]identityschema.Attribute{
			...
			"name": identityschema.StringAttribute{
				RequiredForImport: true,
				Description:       "Scalyr logging endpoint name.",
			},
		},
	}
}
...
func (r *Resource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fastly service Scalyr logging endpoint resource. Writes directly to the specified writable service version.",
...
func (r *Resource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	tflog.Debug(ctx, "Creating Fastly Scalyr logging endpoint", map[string]any{
		"service_id": plan.Service.ValueString(),
		"version":    plan.Version.ValueInt64(),
		"name":       service.StringValue(plan.Name),
	})

	if err := validation.EnsureServiceTypeSupported(ctx, r.providerData.TypeChecker, plan.Service.ValueString(), "fastly_service_logging_scalyr", service.TypeVCL, service.TypeCompute); err != nil {
	...
	l, err := r.providerData.Client.CreateScalyr(ctx, input)
	if err != nil {
		resp.Diagnostics.AddError("Error creating Scalyr logging endpoint", err.Error())
		return
	}
...
func (r *Resource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	tflog.Debug(ctx, "Reading Fastly Scalyr logging endpoint from API", map[string]any{
		"service_id": state.Service.ValueString(),
		"version":    state.Version.ValueInt64(),
		"name":       state.Name.ValueString(),
	})

	l, err := r.providerData.Client.GetScalyr(ctx, &fastly.GetScalyrInput{
	...
	if err != nil {
		if errors.IsNotFound(err) {
			tflog.Warn(ctx, "Scalyr logging endpoint not found, removing from state", map[string]any{
				"service_id": state.Service.ValueString(),
				"name":       state.Name.ValueString(),
			})
		...
		resp.Diagnostics.AddError("Error reading Scalyr logging endpoint", err.Error())
		return
	}
...
func (r *Resource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	tflog.Debug(ctx, "Updating Fastly Scalyr logging endpoint", map[string]any{
		"service_id": opts.ServiceID,
		"version":    opts.ServiceVersion,
		"name":       opts.Name,
	})

	l, err := r.providerData.Client.UpdateScalyr(ctx, opts)
	if err != nil {
		resp.Diagnostics.AddError("Error updating Scalyr logging endpoint", err.Error())
		return
	}
...
func (r *Resource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting Fastly Scalyr logging endpoint", map[string]any{
		"service_id": state.Service.ValueString(),
		"version":    state.Version.ValueInt64(),
		"name":       state.Name.ValueString(),
	})

	if err := validation.EnsureServiceTypeSupported(ctx, r.providerData.TypeChecker, state.Service.ValueString(), "fastly_service_logging_scalyr", service.TypeVCL, service.TypeCompute); err != nil {
		resp.Diagnostics.AddError("Unsupported service type", err.Error())
		return
	}
	...
	err := r.providerData.Client.DeleteScalyr(ctx, &fastly.DeleteScalyrInput{
		ServiceID:      state.Service.ValueString(),
		ServiceVersion: int(state.Version.ValueInt64()),
		Name:           state.Name.ValueString(),
	})
	if err != nil {
		...
		resp.Diagnostics.AddError("Error deleting Scalyr logging endpoint", err.Error())
	}
}
...
func (r *Resource) importByCompositeID(ctx context.Context, id string, resp *resource.ImportStateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			...
			"Expected import ID in format: service_id/version/name\n"+
				"For example: service123/3/my-scalyr-logger\n\n"+
				"Error: "+err.Error(),
		)
		return
	}

	tflog.Debug(ctx, "Importing Scalyr logging endpoint", map[string]any{
	...
	if err != nil {
		resp.Diagnostics.AddError("Error importing Scalyr logging endpoint", err.Error())
		return
	}
