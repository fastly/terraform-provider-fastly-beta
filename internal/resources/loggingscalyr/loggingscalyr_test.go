...
func defaultNestedModel() NestedModel {
	return NestedModel{
		...
		Format:            types.StringValue(constants.LoggingScalyrDefaultFormat),
...
func defaultCommonModel() commonModel {
	return commonModel{
		...
		ProjectID:        types.StringValue(DefaultProjectID),
...
func fullNestedModel() NestedModel {
	...
	m.Name = types.StringValue("test-scalyr")
	m.Authentication = NewAuthenticationObject(types.StringValue("scalyr-token"))
	m.ProjectID = types.StringValue("my-project")
...
func TestFlatten(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, m *Model) {
				assert.Equal(t, types.StringValue("service-123-5-test-scalyr"), m.ID)
				assert.Equal(t, types.StringValue("service-123"), m.Service)
				assert.Equal(t, types.Int64Value(5), m.Version)
				assert.Equal(t, types.StringValue("test-scalyr"), m.Name)
			},
		},
	}
...
func TestBuildCreateInput(t *testing.T) {
	tests := []struct {
		...
		version   int
		model     NestedModel
		validate  func(t *testing.T, input *fastly.CreateScalyrInput)
	}{
		{
			name:      "minimal model",
			...
			validate: func(t *testing.T, input *fastly.CreateScalyrInput) {
				...
				assert.Equal(t, "test-scalyr", *input.Name)
				assert.Equal(t, "scalyr-token", *input.Token)
				assert.Equal(t, "logplex", *input.ProjectID)
				assert.Equal(t, "US", *input.Region)
				assert.Equal(t, "none", *input.ProcessingRegion)
				assert.Equal(t, constants.LoggingScalyrDefaultFormat, *input.Format)
		...
		},
		{
			...
			validate: func(t *testing.T, input *fastly.CreateScalyrInput) {
				assert.Equal(t, "test-scalyr", *input.Name)
				assert.Equal(t, "scalyr-token", *input.Token)
				assert.Equal(t, "my-project", *input.ProjectID)
...
func TestBuildUpdateInput(t *testing.T) {
	...
	assert.Equal(t, 10, input.ServiceVersion)
	assert.Equal(t, "test-scalyr", input.Name)
	assert.Equal(t, "test-scalyr", *input.NewName)
	assert.Equal(t, "scalyr-token", *input.Token)
	assert.Equal(t, "my-project", *input.ProjectID)
	assert.Equal(t, "EU", *input.Region)
	assert.Equal(t, "eu", *input.ProcessingRegion)
...
func TestClearVCLOnlyCreateFields(t *testing.T) {
	input := &fastly.CreateScalyrInput{
...
func TestResetVCLOnlyToDefaults(t *testing.T) {
	...
	m := FlattenToNestedModel(&fastly.Scalyr{
		Name:              new("test-scalyr"),
		Token:             new("scalyr-token"),
	...
	assert.Equal(t, constants.LoggingScalyrDefaultFormat, m.Format.ValueString())
	...
	assert.Equal(t, "test-scalyr", m.Name.ValueString())
...
func TestModelsEqual(t *testing.T) {
	tests := []struct {
		...
		{
			name: "different project_id",
			a: func() NestedModel {
				m := minimalNestedModel()
				m.ProjectID = types.StringValue("project-a")
				return m
			}(),
			b: func() NestedModel {
				m := minimalNestedModel()
				m.ProjectID = types.StringValue("project-b")
				return m
			}(),
			expected: false,
		},
...
// `logging_scalyr { format = ... }` inside fastly_service_compute_auto
...
func TestComputeAttributesOmitsVCLOnly(t *testing.T) {
	...
	for _, name := range []string{"name", "authentication", "project_id", "region", "processing_region"} {
...
func TestAuthenticationAttribute(t *testing.T) {
	...
	assert.True(t, auth.Required, "authentication is Required: there is no FASTLY_SCALYR_* env var to default it from")
	...
	assert.True(t, token.Sensitive, "the Scalyr token must never be rendered in plan output")

	assert.Len(t, auth.Attributes, 1, "authentication holds only token for Scalyr")
}
...
func TestTokenAccessor(t *testing.T) {
	assert.Equal(t, "scalyr-token", minimalNestedModel().Token().ValueString())
...
// at plan time: region US/EU, processing_region none/us/eu, format_version 1-2,
...
func TestSchemaValidators(t *testing.T) {
	...
	stringCases := []struct {
		...
		{"region rejects empty", "region", "", false},
...
func TestValidateConditionReferences(t *testing.T) {
	...
	t.Run("references a condition that isn't configured", func(t *testing.T) {
		...
		if assert.Error(t, err) {
			assert.Contains(t, err.Error(), `"test-scalyr"`)
