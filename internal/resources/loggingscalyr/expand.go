...
func buildCommonCreateInput(serviceID string, version int, m commonModel) *fastly.CreateScalyrInput {
...
func BuildCreateInput(serviceID string, version int, m NestedModel) *fastly.CreateScalyrInput {
...
func BuildComputeCreateInput(serviceID string, version int, m ComputeNestedModel) *fastly.CreateScalyrInput {
...
// service-type-specific fields on top of this.
func buildCommonUpdateInput(serviceID string, version int, m commonModel) *fastly.UpdateScalyrInput {
	...
	// project_id and region default to non-empty values ("logplex"/"US"), so
	// they are never empty and fastly.NullString always sends them.
	input.ProjectID = fastly.NullString(service.StringValue(m.ProjectID))
	input.Region = fastly.NullString(service.StringValue(m.Region))
	input.ProcessingRegion = fastly.NullString(service.StringValue(m.ProcessingRegion))
...
func BuildUpdateInput(serviceID string, version int, m NestedModel) *fastly.UpdateScalyrInput {
	...
	// schema.go). UpdateScalyrInput.Placement is a *Nullable[string]
...
// sets format, format_version, placement, or response_condition, since those
// only affect generated VCL and Compute services don't have any.
func BuildComputeUpdateInput(serviceID string, version int, m ComputeNestedModel) *fastly.UpdateScalyrInput {
...
// response_condition on a CreateScalyrInput. The standalone
// fastly_service_logging_scalyr resource shares one schema across both
...
func ClearVCLOnlyCreateFields(input *fastly.CreateScalyrInput) {
	input.Format = nil
...
// ClearVCLOnlyUpdateFields is ClearVCLOnlyCreateFields for UpdateScalyrInput.
func ClearVCLOnlyUpdateFields(input *fastly.UpdateScalyrInput) {
	input.Format = nil
	input.FormatVersion = nil
	input.Placement = nil
	input.ResponseCondition = nil
