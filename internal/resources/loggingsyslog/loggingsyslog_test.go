...
func defaultNestedModel() NestedModel {
	return NestedModel{
		...
		Format:            types.StringValue(constants.LoggingSyslogDefaultFormat),
...
func fullNestedModel() NestedModel {
	...
	m.Name = types.StringValue("test-syslog")
	m.Address = types.StringValue("syslog.example.com")
	m.Port = types.Int64Value(9000)
	m.Authentication = NewAuthenticationObject(types.StringValue("syslog-token"))
	m.TLS = NewTLSObject(
		types.StringValue("ca-cert"),
		types.StringValue("client-cert"),
		types.StringValue("client-key"),
		types.StringValue("syslog.example.com"),
	)
...
func TestFlatten(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, m *Model) {
				assert.Equal(t, types.StringValue("service-123-5-test-syslog"), m.ID)
				assert.Equal(t, types.StringValue("service-123"), m.Service)
				assert.Equal(t, types.Int64Value(5), m.Version)
				assert.Equal(t, types.StringValue("test-syslog"), m.Name)
			},
		},
	}
...
func TestBuildCreateInput(t *testing.T) {
	tests := []struct {
		...
		version   int
		model     NestedModel
		validate  func(t *testing.T, input *fastly.CreateSyslogInput)
	}{
		{
			name:      "minimal model",
			...
			validate: func(t *testing.T, input *fastly.CreateSyslogInput) {
				...
				assert.Equal(t, "test-syslog", *input.Name)
				assert.Equal(t, "syslog.example.com", *input.Address)
				assert.Equal(t, DefaultPort, *input.Port)
				...
				assert.Equal(t, constants.LoggingSyslogDefaultFormat, *input.Format)
				assert.Nil(t, input.Token, "empty token must be omitted, not sent as \"\"")
		...
		},
		{
			...
			validate: func(t *testing.T, input *fastly.CreateSyslogInput) {
				assert.Equal(t, "test-syslog", *input.Name)
				assert.Equal(t, "syslog.example.com", *input.Address)
				assert.Equal(t, 9000, *input.Port)
				assert.Equal(t, "syslog-token", *input.Token)
				...
				assert.Equal(t, "syslog.example.com", *input.TLSHostname)
...
func TestBuildUpdateInput(t *testing.T) {
	...
	assert.Equal(t, 10, input.ServiceVersion)
	assert.Equal(t, "test-syslog", input.Name)
	assert.Equal(t, "test-syslog", *input.NewName)
	assert.Equal(t, "syslog.example.com", *input.Address)
	assert.Equal(t, "syslog-token", *input.Token)
	assert.Equal(t, "ca-cert", *input.TLSCACert)
	assert.Equal(t, "client-key", *input.TLSClientKey)
...
func TestBuildUpdateInputClearsClearableFields(t *testing.T) {
	...
	assert.NotNil(t, input.Token, "token must be sent even when empty")
	assert.Equal(t, "", *input.Token)
...
func TestResetVCLOnlyToDefaults(t *testing.T) {
	...
	m := FlattenToNestedModel(&fastly.Syslog{
		Name:              new("test-syslog"),
		Address:           new("syslog.example.com"),
	...
	assert.Equal(t, constants.LoggingSyslogDefaultFormat, m.Format.ValueString())
	...
	assert.Equal(t, "test-syslog", m.Name.ValueString())
	assert.Equal(t, "syslog.example.com", m.Address.ValueString())
	...
	tests := []struct {
		...
		},
		{
			...
			a: func() NestedModel {
				...
				m.Address = types.StringValue("one.example.com")
				return m
			}(),
			b: func() NestedModel {
				m := minimalNestedModel()
				m.Address = types.StringValue("two.example.com")
				return m
			}(),
			expected: false,
		},
...
// `logging_syslog { format = ... }` inside fastly_service_compute_auto fail at
...
func TestComputeAttributesOmitsVCLOnly(t *testing.T) {
	...
	for _, name := range []string{"name", "address", "port", "authentication", "tls", "use_tls", "message_type", "processing_region"} {
...
// Computed `authentication` object with a single Sensitive `token` inside,
// defaulting to "" — the live (SDKv2) provider has no environment variable
// for this token.
func TestAuthenticationAttribute(t *testing.T) {
	...
	assert.True(t, token.Sensitive, "the Syslog token must never be rendered in plan output")

	assert.Len(t, auth.Attributes, 1, "authentication holds only token for Syslog")
}
...
// material used to authenticate to the Syslog server via mutual TLS.
...
func TestTokenAccessor(t *testing.T) {
	assert.Equal(t, "syslog-token", fullNestedModel().Token().ValueString())
...
func TestTLSAccessors(t *testing.T) {
	...
	assert.Equal(t, "syslog.example.com", full.TLSHostname().ValueString())
...
// TestSchemaValidators pins the accepted values for the remaining validators,
// so a change to an enum member or a bound is a test failure rather than a
// surprise at plan time: message_type classic/loggly/logplex/blank,
// processing_region none/us/eu, format_version 1-2, placement "none" only,
// format capped at 12288.
...
func TestValidateConditionReferences(t *testing.T) {
	...
	t.Run("references a condition that isn't configured", func(t *testing.T) {
		...
		if assert.Error(t, err) {
			assert.Contains(t, err.Error(), `"test-syslog"`)
