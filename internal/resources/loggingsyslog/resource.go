...
func (r *Resource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_service_logging_syslog"
}
	...
	resp.IdentitySchema = identityschema.Schema{
		Attributes: map[string]identityschema.Attribute{
			...
			"name": identityschema.StringAttribute{
				RequiredForImport: true,
				Description:       "Syslog logging endpoint name.",
			},
		},
	}
}
...
func (r *Resource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fastly service Syslog logging endpoint resource. Writes directly to the specified writable service version.",
...
func (r *Resource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	tflog.Debug(ctx, "Creating Fastly Syslog logging endpoint", map[string]any{
		"service_id": plan.Service.ValueString(),
		"version":    plan.Version.ValueInt64(),
		"name":       service.StringValue(plan.Name),
	})

	if err := validation.EnsureServiceTypeSupported(ctx, r.providerData.TypeChecker, plan.Service.ValueString(), "fastly_service_logging_syslog", service.TypeVCL, service.TypeCompute); err != nil {
	...
	s, err := r.providerData.Client.CreateSyslog(ctx, input)
	if err != nil {
		resp.Diagnostics.AddError("Error creating Syslog logging endpoint", err.Error())
		return
	}
...
func (r *Resource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	tflog.Debug(ctx, "Reading Fastly Syslog logging endpoint from API", map[string]any{
		"service_id": state.Service.ValueString(),
		"version":    state.Version.ValueInt64(),
		"name":       state.Name.ValueString(),
	})

	s, err := r.providerData.Client.GetSyslog(ctx, &fastly.GetSyslogInput{
	...
	if err != nil {
		if errors.IsNotFound(err) {
			tflog.Warn(ctx, "Syslog logging endpoint not found, removing from state", map[string]any{
				"service_id": state.Service.ValueString(),
				"name":       state.Name.ValueString(),
			})
		...
		resp.Diagnostics.AddError("Error reading Syslog logging endpoint", err.Error())
		return
	}
...
func (r *Resource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	tflog.Debug(ctx, "Updating Fastly Syslog logging endpoint", map[string]any{
		"service_id": opts.ServiceID,
		"version":    opts.ServiceVersion,
		"name":       opts.Name,
	})

	s, err := r.providerData.Client.UpdateSyslog(ctx, opts)
	if err != nil {
		resp.Diagnostics.AddError("Error updating Syslog logging endpoint", err.Error())
		return
	}
...
func (r *Resource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting Fastly Syslog logging endpoint", map[string]any{
		"service_id": state.Service.ValueString(),
		"version":    state.Version.ValueInt64(),
		"name":       state.Name.ValueString(),
	})

	if err := validation.EnsureServiceTypeSupported(ctx, r.providerData.TypeChecker, state.Service.ValueString(), "fastly_service_logging_syslog", service.TypeVCL, service.TypeCompute); err != nil {
		resp.Diagnostics.AddError("Unsupported service type", err.Error())
		return
	}
	...
	err := r.providerData.Client.DeleteSyslog(ctx, &fastly.DeleteSyslogInput{
		ServiceID:      state.Service.ValueString(),
		ServiceVersion: int(state.Version.ValueInt64()),
		Name:           state.Name.ValueString(),
	})
	if err != nil {
		...
		resp.Diagnostics.AddError("Error deleting Syslog logging endpoint", err.Error())
	}
}
...
func (r *Resource) importByCompositeID(ctx context.Context, id string, resp *resource.ImportStateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			...
			"Expected import ID in format: service_id/version/name\n"+
				"For example: service123/3/my-syslog-logger\n\n"+
				"Error: "+err.Error(),
		)
		return
	}

	tflog.Debug(ctx, "Importing Syslog logging endpoint", map[string]any{
	...
	if err != nil {
		resp.Diagnostics.AddError("Error importing Syslog logging endpoint", err.Error())
		return
	}
