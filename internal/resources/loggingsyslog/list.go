...
func (l *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "List all Syslog logging endpoints across all Fastly CDN and Compute services at their active version, or latest version when no active version exists.",
...
func (l *ListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	tflog.Debug(ctx, "Listing Fastly Syslog logging endpoints")
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, svc := range services {
			...
			loggers, err := l.client.ListSyslogs(ctx, &fastly.ListSyslogsInput{
				ServiceID:      serviceID,
				ServiceVersion: version,
			})
			if err != nil {
				tflog.Warn(ctx, "Error listing Syslog logging endpoints for service", map[string]any{
					"service_id": serviceID,
					"error":      err.Error(),
				})
				continue
			...
			for _, s := range loggers {
				...
				result.Diagnostics.Append(result.Identity.Set(ctx, &IdentityModel{
					...
					Name:      types.StringValue(fastly.ToValue(s.Name)),
				})...)
...
// accept.
func setResourceAttrs(ctx context.Context, result *list.ListResult, s *fastly.Syslog, serviceID string, version int, serviceType string) diag.Diagnostics {
