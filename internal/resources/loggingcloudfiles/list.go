...
func (l *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "List all Cloud Files logging endpoints across all Fastly CDN and Compute services at their active version, or latest version when no active version exists.",
...
func (l *ListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	tflog.Debug(ctx, "Listing Fastly Cloudfiles logging endpoints")
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, svc := range services {
			...
			loggers, err := l.client.ListCloudfiles(ctx, &fastly.ListCloudfilesInput{
				ServiceID:      serviceID,
				ServiceVersion: version,
			})
			if err != nil {
				tflog.Warn(ctx, "Error listing Cloudfiles logging endpoints for service", map[string]any{
					"service_id": serviceID,
					"error":      err.Error(),
				})
				continue
			...
			for _, c := range loggers {
				...
				result.Diagnostics.Append(result.Identity.Set(ctx, &IdentityModel{
					...
					Name:      types.StringValue(fastly.ToValue(c.Name)),
				})...)

				if req.IncludeResource {
					result.Diagnostics.Append(setResourceAttrs(ctx, &result, c, serviceID, version, fastly.ToValue(svc.Type))...)
				}
...
// accept.
func setResourceAttrs(ctx context.Context, result *list.ListResult, c *fastly.Cloudfiles, serviceID string, version int, serviceType string) diag.Diagnostics {
