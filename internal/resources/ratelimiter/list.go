...
func (l *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "List all rate limiters across all Fastly CDN services at their active version, or latest version when no active version exists.",
...
func (l *ListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	tflog.Debug(ctx, "Listing Fastly service rate limiters")
	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, svc := range services {
			...
			erls, err := l.client.ListERLs(ctx, &fastly.ListERLsInput{
				ServiceID:      serviceID,
				ServiceVersion: version,
			})
			if err != nil {
				tflog.Warn(ctx, "Error listing rate limiters for service", map[string]any{
					"service_id": serviceID,
					"error":      err.Error(),
				})
				continue
			...
			for _, e := range erls {
				...
				result.DisplayName = service.ToGeneratedResourceName(fastly.ToValue(svc.Name), serviceID, *e.Name)
				result.Diagnostics.Append(result.Identity.Set(ctx, &IdentityModel{
					ServiceID: types.StringValue(serviceID),
					Name:      types.StringValue(fastly.ToValue(e.Name)),
				})...)

				if req.IncludeResource {
					result.Diagnostics.Append(setResourceAttrs(ctx, &result, e, serviceID, version)...)
				}
...
func setResourceAttrs(ctx context.Context, result *list.ListResult, e *fastly.ERL, serviceID string, version int) diag.Diagnostics {
	var diags diag.Diagnostics

	id := serviceID + "/" + strconv.Itoa(version) + "/" + fastly.ToValue(e.Name)

	model := Model{
		NestedModel: ops{}.ToModel(e),
