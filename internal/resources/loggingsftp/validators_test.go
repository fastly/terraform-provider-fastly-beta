...
// TestRequireOneCredential covers the plan-time replacement for the legacy
// SDKv2 provider's apply-time-only Create check ("either password or
// secret_key must be set").
func TestRequireOneCredential(t *testing.T) {
	tests := []struct {
		name      string
		user      types.String
		...
		{
			name:      "neither password nor secret_key set",
		...
		},
		{
			...
			user:      types.StringValue("user"),
			password:  types.StringNull(),
			secretKey: types.StringValue("key"),
			wantError: false,
		},
		{
			name:      "both set",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			obj := NewAuthenticationObject(tt.user, tt.password, tt.secretKey)
			...
			requireOneCredential{}.ValidateObject(context.Background(), req, resp)

			assert.Equal(t, tt.wantError, resp.Diagnostics.HasError())
		})
	}

	t.Run("null object is skipped", func(t *testing.T) {
		req := validator.ObjectRequest{
			Path:        path.Root("authentication"),
	...
	t.Run("unknown object is skipped", func(t *testing.T) {
		req := validator.ObjectRequest{
			Path:        path.Root("authentication"),
			ConfigValue: types.ObjectUnknown(authenticationAttributeTypes),
		}
		resp := &validator.ObjectResponse{}
