...
// credentialOrNull maps an unset password/secret_key ("" from the API) to
// null, matching the null an unconfigured plan produces for these
// Optional-with-no-default fields — otherwise apply fails with "inconsistent
// values for sensitive attribute".
func credentialOrNull(v *string) types.String {
	if v == nil || *v == "" {
...
func FlattenToNestedModel(f *fastly.SFTP) NestedModel {
	m := NestedModel{}
	...
	m.Name = types.StringValue(fastly.ToValue(f.Name))
	m.Address = types.StringValue(fastly.ToValue(f.Address))
	m.Authentication = NewAuthenticationObject(
		service.StringPointerOrDefault(f.User, ""),
		credentialOrNull(f.Password),
		credentialOrNull(f.SecretKey),
	)
	m.Path = service.StringPointerOrDefault(f.Path, "")
	m.SSHKnownHosts = service.StringPointerOrDefault(f.SSHKnownHosts, "")
	m.Port = service.Int64PointerOrDefault(f.Port, DefaultPort)
	m.Period = service.Int64PointerOrDefault(f.Period, DefaultPeriod)
	m.GzipLevel = service.Int64PointerOrDefault(f.GzipLevel, DefaultGzipLevel)
	m.CompressionCodec = service.StringPointerOrDefault(f.CompressionCodec, DefaultCompressionCodec)
	m.Format = service.StringPointerOrDefault(f.Format, constants.LoggingSFTPDefaultFormat)
	m.FormatVersion = service.Int64PointerOrDefault(f.FormatVersion, DefaultFormatVersion)
	m.MessageType = service.StringPointerOrDefault(f.MessageType, DefaultMessageType)
	m.TimestampFormat = service.StringPointerOrDefault(f.TimestampFormat, DefaultTimestampFormat)
	m.Placement = service.StringPointerOrNull(f.Placement)
	m.ResponseCondition = service.StringPointerOrDefault(f.ResponseCondition, DefaultResponseCondition)
	m.PublicKey = service.StringPointerOrDefault(f.PublicKey, DefaultPublicKey)
	m.ProcessingRegion = service.StringPointerOrDefault(f.ProcessingRegion, DefaultProcessingRegion)
...
func ResetVCLOnlyToDefaults(m *NestedModel) {
	m.Format = types.StringValue(constants.LoggingSFTPDefaultFormat)
...
func FlattenToComputeNestedModel(f *fastly.SFTP) ComputeNestedModel {
	return ComputeNestedModel{commonModel: FlattenToNestedModel(f).commonModel}
}
...
func flatten(ctx context.Context, f *fastly.SFTP, m *Model) {
	if f == nil {
		tflog.Warn(ctx, "flatten called with nil SFTP logging endpoint")
		return
	}

	id := fastly.ToValue(f.ServiceID) + "/" + strconv.Itoa(fastly.ToValue(f.ServiceVersion)) + "/" + fastly.ToValue(f.Name)
	m.ID = types.StringValue(id)
	m.Service = types.StringValue(fastly.ToValue(f.ServiceID))
	m.Version = types.Int64Value(int64(fastly.ToValue(f.ServiceVersion)))

	m.NestedModel = FlattenToNestedModel(f)

	tflog.Debug(ctx, "Flattened SFTP logging endpoint state", map[string]any{
		"id":      id,
