...
func defaultNestedModel() NestedModel {
	return NestedModel{
		...
		Format:            types.StringValue(constants.LoggingSFTPDefaultFormat),
...
func defaultCommonModel() commonModel {
	return commonModel{
		...
		Authentication:   NewAuthenticationObject(types.StringValue(""), types.StringNull(), types.StringNull()),
		...
		SSHKnownHosts:    types.StringValue(""),
...
func fullNestedModel() NestedModel {
	...
	m.Name = types.StringValue("test-sftp")
	m.Address = types.StringValue("sftp.example.com")
	m.Authentication = NewAuthenticationObject(
		types.StringValue("test-user"),
		types.StringValue("test-password"),
	...
	m.SSHKnownHosts = types.StringValue("sftp.example.com ssh-rsa AAAA...")
	...
	tests := []struct {
		...
		},
		{
			...
			api: &fastly.SFTP{
				...
				Path:          new("/"),
				SSHKnownHosts: new("sftp.example.com ssh-rsa AAAA..."),
			},
...
func TestFlatten(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, m *Model) {
				assert.Equal(t, types.StringValue("service-123/5/test-sftp"), m.ID)
				assert.Equal(t, types.StringValue("service-123"), m.Service)
				assert.Equal(t, types.Int64Value(5), m.Version)
				assert.Equal(t, types.StringValue("test-sftp"), m.Name)
			},
		},
	}
...
func TestBuildCreateInput(t *testing.T) {
	tests := []struct {
		...
		version   int
		model     NestedModel
		validate  func(t *testing.T, input *fastly.CreateSFTPInput)
	}{
		{
			name:      "minimal model",
			...
			validate: func(t *testing.T, input *fastly.CreateSFTPInput) {
				...
				assert.Equal(t, "test-sftp", *input.Name)
				assert.Equal(t, "sftp.example.com", *input.Address)
				assert.Equal(t, "test-user", *input.User)
				assert.Equal(t, "test-password", *input.Password)
				assert.Nil(t, input.SecretKey, "an unset secret_key must not be sent alongside a configured password")
				assert.Equal(t, "/", *input.Path)
				assert.Equal(t, "sftp.example.com ssh-rsa AAAA...", *input.SSHKnownHosts)
				assert.Equal(t, "none", *input.ProcessingRegion)
				assert.Equal(t, constants.LoggingSFTPDefaultFormat, *input.Format)
...
func TestBuildUpdateInput(t *testing.T) {
	...
	assert.Equal(t, 10, input.ServiceVersion)
	assert.Equal(t, "test-sftp", input.Name)
	assert.Equal(t, "test-sftp", *input.NewName)
	assert.Equal(t, "sftp.example.com", *input.Address)
	assert.Equal(t, "test-user", *input.User)
	assert.Equal(t, "test-password", *input.Password)
	assert.Equal(t, "test-secret-key", *input.SecretKey)
...
// and public_key are always sent as concrete values on update — even when
...
// the same way, but as an explicit JSON null rather than an empty string — see
// BuildUpdateInput. password/secret_key are the opposite: an unset one must
// never be sent, even as "", because the API treats a present-but-blank
// secret_key as "passed" and prefers it over a real password — see
// buildCommonCreateInput.
...
func TestResetVCLOnlyToDefaults(t *testing.T) {
	...
	m := FlattenToNestedModel(&fastly.SFTP{
		Name:              new("test-sftp"),
		Address:           new("sftp.example.com"),
		User:              new("test-user"),
		Password:          new("test-password"),
	...
	assert.Equal(t, constants.LoggingSFTPDefaultFormat, m.Format.ValueString())
	...
	assert.Equal(t, "test-sftp", m.Name.ValueString())
	assert.Equal(t, "test-user", m.User().ValueString())
	assert.Equal(t, "test-password", m.Password().ValueString())
...
func TestCredentialAccessors(t *testing.T) {
	...
	assert.Equal(t, "test-password", minimalNestedModel().Password().ValueString())
	assert.Equal(t, "test-secret-key", fullNestedModel().SecretKey().ValueString())
	...
	tests := map[string]types.Object{
		...
		"null values":    NewAuthenticationObject(types.StringNull(), types.StringNull(), types.StringNull()),
		"unknown values": NewAuthenticationObject(types.StringUnknown(), types.StringUnknown(), types.StringUnknown()),
	}
...
func TestModelsEqual(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			a: func() NestedModel {
				m := minimalNestedModel()
				m.Authentication = NewAuthenticationObject(types.StringValue("test-user"), types.StringValue("pw-1"), types.StringValue(""))
				return m
			}(),
			b: func() NestedModel {
				m := minimalNestedModel()
				m.Authentication = NewAuthenticationObject(types.StringValue("test-user"), types.StringValue("pw-2"), types.StringValue(""))
				return m
			}(),
			expected: false,
		},
		{
			...
			a: func() NestedModel {
				m := minimalNestedModel()
				m.Authentication = NewAuthenticationObject(types.StringValue("test-user"), types.StringValue(""), types.StringValue("key-1"))
				return m
			}(),
			b: func() NestedModel {
				m := minimalNestedModel()
				m.Authentication = NewAuthenticationObject(types.StringValue("test-user"), types.StringValue(""), types.StringValue("key-2"))
				return m
			}(),
			expected: false,
		},
...
func TestComputeModelsEqual(t *testing.T) {
	...
	b.Address = types.StringValue("other.example.com")
		...
		},
		{
			...
			b: []NestedModel{
				func() NestedModel {
					m := minimalNestedModel()
					m.Name = types.StringValue("a")
					m.Address = types.StringValue("other.example.com")
					return m
				}(),
			},
			expected: false,
...
func TestComputeEqual(t *testing.T) {
	...
	b[0].Address = types.StringValue("other.example.com")
...
// `logging_sftp { format = ... }` inside fastly_service_compute_auto fail at
...
func TestComputeAttributesOmitsVCLOnly(t *testing.T) {
	...
	for _, name := range []string{"name", "address", "authentication", "path", "ssh_known_hosts", "processing_region"} {
		assert.Contains(t, compute, name)
		assert.Contains(t, common, name)
	}

	// user/password/secret_key are nested under authentication, never
	// top-level attributes.
	for _, name := range []string{"user", "password", "secret_key"} {
		assert.NotContains(t, compute, name)
...
// `authentication` object with a Required `user`, and Optional+Sensitive
// `password`/`secret_key` — at least one of which requireOneCredential
// enforces must be set, mirroring the legacy provider's apply-time-only
// "either password or secret_key must be set" check.
func TestAuthenticationAttribute(t *testing.T) {
	...
	assert.True(t, auth.Required, "authentication is Required: there is no FASTLY_SFTP_* environment variable to default it from")
	assert.False(t, auth.Computed)
	assert.Nil(t, auth.Default, "a Required object must not carry a Default")
	require.Len(t, auth.Validators, 1)
	assert.IsType(t, requireOneCredential{}, auth.Validators[0])
	...
	assert.True(t, password.Optional, "password is Optional: either it or secret_key may be set")
	assert.True(t, password.Sensitive, "the SFTP password must never be rendered in plan output")
	...
	assert.True(t, secretKey.Optional, "secret_key is Optional: either it or password may be set")
	assert.True(t, secretKey.Sensitive, "the SFTP secret_key must never be rendered in plan output")

	assert.Len(t, auth.Attributes, 3, "authentication holds user, password, and secret_key for SFTP")
}
...
func TestSchemaValidators(t *testing.T) {
	...
	// name, address, path, ssh_known_hosts, and response_condition accept any
	// string; assert that rather than leaving it implicit.
	...
	assert.Empty(t, attrs["path"].(schema.StringAttribute).Validators)
	assert.Empty(t, attrs["ssh_known_hosts"].(schema.StringAttribute).Validators)
...
func TestValidateConditionReferences(t *testing.T) {
	...
	t.Run("references a condition that isn't configured", func(t *testing.T) {
		...
		if assert.Error(t, err) {
			assert.Contains(t, err.Error(), `"test-sftp"`)
