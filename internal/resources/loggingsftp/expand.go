...
func buildCommonCreateInput(serviceID string, version int, m commonModel) *fastly.CreateSFTPInput {
	input := &fastly.CreateSFTPInput{
		...
		Path:           new(service.StringValue(m.Path)),
		SSHKnownHosts:  new(service.StringValue(m.SSHKnownHosts)),
	}

	// Only send whichever credential is actually configured. The API prefers
	// secret_key whenever it's present, even blank, so sending both would make
	// an empty secret_key win over a real password.
...
func BuildCreateInput(serviceID string, version int, m NestedModel) *fastly.CreateSFTPInput {
...
// response_condition on a CreateSFTPInput. The standalone
// fastly_service_logging_sftp resource shares one schema across both service
...
func ClearVCLOnlyCreateFields(input *fastly.CreateSFTPInput) {
	input.Format = nil
...
// ClearVCLOnlyUpdateFields is ClearVCLOnlyCreateFields for UpdateSFTPInput.
func ClearVCLOnlyUpdateFields(input *fastly.UpdateSFTPInput) {
	input.Format = nil
	input.FormatVersion = nil
	input.Placement = nil
	input.ResponseCondition = nil
...
func BuildComputeCreateInput(serviceID string, version int, m ComputeNestedModel) *fastly.CreateSFTPInput {
...
// service-type-specific fields on top of this.
func buildCommonUpdateInput(serviceID string, version int, m commonModel) *fastly.UpdateSFTPInput {
	...
	// Only the configured credential is sent — see buildCommonCreateInput.
	input.Password = fastly.NullString(service.StringValue(m.Password()))
	input.SecretKey = fastly.NullString(service.StringValue(m.SecretKey()))
...
// sets format, format_version, placement, or response_condition, since those
// only affect generated VCL and Compute services don't have any.
func BuildComputeUpdateInput(serviceID string, version int, m ComputeNestedModel) *fastly.UpdateSFTPInput {
	return buildCommonUpdateInput(serviceID, version, m.commonModel)
}

func BuildUpdateInput(serviceID string, version int, m NestedModel) *fastly.UpdateSFTPInput {
	...
	// schema.go). UpdateSFTPInput.Placement is a *Nullable[string] specifically
