...
// normalization gap as TestResetVCLOnlyToDefaults (see loggingpapertrail_test.go),
...
func TestSetResourceAttrs_computeResetsVCLOnlyFields(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantFormat:        types.StringValue(constants.LoggingPapertrailDefaultFormat),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			diags := setResourceAttrs(ctx, &result, p, "service-id", 1, tt.serviceType)
