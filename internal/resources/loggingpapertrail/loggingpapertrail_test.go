...
func defaultNestedModel() NestedModel {
	return NestedModel{
		...
		Format:            types.StringValue(constants.LoggingPapertrailDefaultFormat),
...
func fullNestedModel() NestedModel {
	...
	m.Name = types.StringValue("test-papertrail")
	m.Address = types.StringValue("logs2.papertrailapp.com")
	m.Port = types.Int64Value(54321)
...
func minimalNestedModel() NestedModel {
	m := defaultNestedModel()
	m.Name = types.StringValue("test-papertrail")
	m.Address = types.StringValue("logs.papertrailapp.com")
	m.Port = types.Int64Value(12345)
	return m
}
	...
	tests := []struct {
		...
		},
		{
			...
			api: &fastly.Papertrail{
				...
				Address: new("logs.papertrailapp.com"),
				Port:    new(12345),
			},
		...
		},
		{
			...
			api: &fastly.Papertrail{
				Name:              new("test-papertrail"),
				Address:           new("logs2.papertrailapp.com"),
				Port:              new(54321),
...
func TestFlatten(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, m *Model) {
				assert.Equal(t, types.StringValue("service-123/5/test-papertrail"), m.ID)
				assert.Equal(t, types.StringValue("service-123"), m.Service)
				assert.Equal(t, types.Int64Value(5), m.Version)
				assert.Equal(t, types.StringValue("test-papertrail"), m.Name)
			},
		},
	}
...
func TestBuildCreateInput(t *testing.T) {
	tests := []struct {
		...
		version   int
		model     NestedModel
		validate  func(t *testing.T, input *fastly.CreatePapertrailInput)
	}{
		{
			name:      "minimal model",
			...
			validate: func(t *testing.T, input *fastly.CreatePapertrailInput) {
				...
				assert.Equal(t, "test-papertrail", *input.Name)
				assert.Equal(t, "logs.papertrailapp.com", *input.Address)
				assert.Equal(t, 12345, *input.Port)
				assert.Equal(t, "none", *input.ProcessingRegion)
				assert.Equal(t, constants.LoggingPapertrailDefaultFormat, *input.Format)
		...
		},
		{
			...
			validate: func(t *testing.T, input *fastly.CreatePapertrailInput) {
				assert.Equal(t, "test-papertrail", *input.Name)
				assert.Equal(t, "logs2.papertrailapp.com", *input.Address)
				assert.Equal(t, 54321, *input.Port)
...
func TestBuildUpdateInput(t *testing.T) {
	...
	assert.Equal(t, 10, input.ServiceVersion)
	assert.Equal(t, "test-papertrail", input.Name)
	assert.Equal(t, "test-papertrail", *input.NewName)
	assert.Equal(t, "logs2.papertrailapp.com", *input.Address)
	assert.Equal(t, 54321, *input.Port)
	assert.Equal(t, "eu", *input.ProcessingRegion)
	assert.Equal(t, "%h %l %u", *input.Format)
...
func TestResetVCLOnlyToDefaults(t *testing.T) {
	...
	m := FlattenToNestedModel(&fastly.Papertrail{
		Name:              new("test-papertrail"),
		Address:           new("logs.papertrailapp.com"),
		Port:              new(12345),
	...
	assert.Equal(t, constants.LoggingPapertrailDefaultFormat, m.Format.ValueString())
	...
	assert.Equal(t, "test-papertrail", m.Name.ValueString())
	assert.Equal(t, "logs.papertrailapp.com", m.Address.ValueString())
	assert.Equal(t, int64(12345), m.Port.ValueInt64())
	...
	tests := []struct {
		...
		},
		{
			...
			a: func() NestedModel {
				...
				m.Address = types.StringValue("one.papertrailapp.com")
				return m
			}(),
			b: func() NestedModel {
				m := minimalNestedModel()
				m.Address = types.StringValue("two.papertrailapp.com")
				return m
			}(),
			expected: false,
		},
...
func TestComputeEqual(t *testing.T) {
	...
	b[0].Port = types.Int64Value(9999)
...
// `logging_papertrail { format = ... }` inside fastly_service_compute_auto fail
...
func TestComputeAttributesOmitsVCLOnly(t *testing.T) {
	...
	for _, name := range []string{"name", "address", "port", "processing_region"} {
...
func TestValidateConditionReferences(t *testing.T) {
	...
	t.Run("references a condition that isn't configured", func(t *testing.T) {
		...
		if assert.Error(t, err) {
			assert.Contains(t, err.Error(), `"test-papertrail"`)
