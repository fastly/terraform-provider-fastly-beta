...
func (d *DataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_tls_private_key_ids"
}

func (d *DataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Use this data source to get the IDs of all available TLS private keys.",
...
func (d *DataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	tflog.Debug(ctx, "Listing Fastly TLS private key IDs")

	var ids []string
	pageNumber := 1
	for {
		list, err := d.client.ListPrivateKeys(ctx, &fastly.ListPrivateKeysInput{
