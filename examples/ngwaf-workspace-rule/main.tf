...
resource "fastly_ngwaf_workspace_rule" "block_ip" {
...
resource "fastly_ngwaf_workspace_rule" "block_admin_without_header" {
  workspace_id = fastly_ngwaf_workspace.example.id
  type         = "request"
...
# A signal-type rule: exclude a signal that's producing a false positive on
# a known-safe path. `signal` rules only ever declare one action:
# `exclude_signal`.
...
# A rate_limit-type rule: block a client once it exceeds a request
# threshold. `rate_limit` references a custom signal by its reference ID;
# this repo has no fastly_ngwaf_workspace_signal resource yet, so the
# reference below is a placeholder for a signal created out of band.
# client_identifiers combines two identifiers here (ip + request_cookie) to
# rate limit per session within an IP.
resource "fastly_ngwaf_workspace_rule" "ip_rate_limit" {
  workspace_id = fastly_ngwaf_workspace.example.id
  type         = "rate_limit"
...
# A templated_signal-type rule: `templated_signal` is the one action type
# whose name collides with a rule `type` value, and the sole action a
# templated_signal rule may declare. description must be an empty string.
