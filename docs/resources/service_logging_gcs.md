...
- `authentication` groups credentials as the other logging endpoints do.
  ...
  environment variable when `FASTLY_GOOGLE_SERVICE_ACCOUNT_NAME` is unset — no
  deprecation warning is emitted for this fallback, since `FASTLY_GCS_ACCOUNT_NAME`
  remains this resource's own, current variable name.
...
- `compression_codec` and `gzip_level` are mutually exclusive; setting both
  is rejected.
- `format` must produce valid JSON. If `format` is not sent, it falls
