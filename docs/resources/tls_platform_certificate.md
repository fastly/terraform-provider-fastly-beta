...
Import does not recover `certificate_body`, `intermediates_blob`, or `allow_untrusted_root` - they
are never returned for an existing certificate, so they are left unset in state after
