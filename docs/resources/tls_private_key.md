...
`private_key.pem` is not returned on read and so is not populated by import; the next `terraform apply` will show a forced replacement unless `private_key.pem` in your configuration exactly matches the key data of the private key you are importing.
