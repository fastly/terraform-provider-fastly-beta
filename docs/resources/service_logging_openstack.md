---
page_title: "fastly_service_logging_openstack Resource - fastly"
subcategory: ""
description: |-
  Fastly service OpenStack logging endpoint resource. Writes directly to the specified writable service version.
---

# fastly_service_logging_openstack (Resource)

Fastly service OpenStack logging endpoint resource. Writes directly to the specified writable service version.

This resource is part of the explicit/default first-class resource family. It
manages an OpenStack real-time logging endpoint on the configured service
...
nested `logging_openstack` block on `fastly_service_cdn_auto` or
`fastly_service_compute_auto` — see "Automatic-lifecycle usage" below.

## Example Usage

```terraform
resource "fastly_service_logging_openstack" "example" {
  ...
  authentication = {
    ...
    access_key = var.openstack_access_key
  }
}
```
...
```terraform
resource "fastly_service_logging_openstack" "full" {
  service_id  = fastly_service_cdn.example.id
  version     = 1
  name        = "openstack-full"
  bucket_name = "my-logs-container"
...
```terraform
resource "fastly_service_logging_openstack" "compute" {
...
Inside the `_auto` service resources, OpenStack logging is a nested block and
...
- `authentication` (Attributes) Authentication credentials for your OpenStack account. (see [below for nested schema](#nestedatt--authentication))
...
```shell
terraform import fastly_service_logging_openstack.example SERVICE_ID/VERSION/ENDPOINT_NAME
```

Example:

```shell
terraform import fastly_service_logging_openstack.example SU1Z0isxPaozGVKXdv0eY/3/openstack-example
```
...
- `authentication` groups credentials as the other logging endpoints do. It is
  required — there is no `FASTLY_OPENSTACK_*` environment variable to default
