...
- `tls` groups the certificate material used for mutual TLS to the HTTPS
  ...
  in plan output.
- `gzip_level` and `compression_codec` are mutually exclusive — setting both
  is rejected. Leave `compression_codec` unset and set
...
- `header_name`/`header_value` add one custom HTTP header to every request the
  ...
  arbitrary header values are accepted here, not only credentials;
