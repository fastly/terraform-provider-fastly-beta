...
Import reads the ACL's current entries into state as-is; it
...
Import reads the current entries into state and sets `manage_entries = true`; it does not delete anything by itself. See [Behavior and Drift Management](#behavior-and-drift-management) above — make sure your `entry` blocks match the imported entries before running `terraform apply`, since `manage_entries = true` will delete any entry missing from your configuration.
